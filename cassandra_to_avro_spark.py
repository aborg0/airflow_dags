...
spark = pyspark.sql.SparkSession.builder.master("local[*]").config("spark.cassandra.auth.username", "cassandra").config("spark.cassandra.auth.password", "cassandra").appName("cassandra_to_avro").getOrCreate()
